                             ...
                             const char *name)
{
    ...
    struct blob_client *client;
    client = get_blob_client_from_fuse_req(req);
                           ...
                           struct fuse_file_info *fi)
{
    ...
    AZLogDebug("reg file: {} isdir: {}",inode->is_regfile(), inode->is_dir());
    assert(!inode->is_dir());
                                ...
                                struct fuse_file_info *fi)
{
    FUSE_STATS_TRACKER(FUSE_WRITE);
    INC_GBL_STATS(fuse_responses_awaited, 1);
    INC_GBL_STATS(app_write_reqs, 1);
