...
public:
    rpc_task(struct blob_client *_client, int _index) :
        client(_client),
        index(_index)
    {
    }
    ...
    void run_read();
    void run_read_blob(size_t size, off_t offset);
    ...
    void reply_attr(const struct stat& attr, double attr_timeout)
    {
        ...
        if (fre != 0) {
            ...
            AZLogInfo("Reply_attr succeeded");
            DEC_GBL_STATS(fuse_responses_awaited, 1);
        }
        ...
        const struct fuse_file_info *file)
    {
         ...
         */
        struct blob_inode *inode = client->get_blob_inode_from_ino(entry->ino).get();
...
private:
    ...
    rpc_task_helper(struct blob_client *client)
    {
        assert(client != nullptr);
        //assert(client != nullptr)
...
public:
    ...
    static rpc_task_helper *get_instance(struct blob_client *client = nullptr)
    {
