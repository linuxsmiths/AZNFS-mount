                             ...
                             uint64_t nlookup)
{
    ...
    struct blob_client *client = get_blob_client_from_fuse_req(req);
    struct blob_inode *inode = client->get_blob_inode_from_ino(ino).get();
...
int main(int argc, char *argv[])
{
    ...
    const std::string connectionString = "AccountName=amsprdsty01enfs25;EndpointSuffix=preprod.core.windows.net;DefaultEndpointsProtocol=https;";
    ...
    if (aznfsc_cfg.auth) {
     ...
     AZLogInfo("Initializing Blob Client...");

    if (!blob_client::get_instance().init(connectionString, containerName)) {
        AZLogError("Failed to initialize Blob Client. Exiting.");
        return -1;
    }
